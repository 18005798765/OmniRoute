...
# OVERLAY_BUST 强制失效 COPY 缓存：overlay 是 untracked/临时 commit，BuildKit 有时不重算 context
ARG OVERLAY_BUST=0
COPY . ./
RUN echo "=== DIAG src/lib OVERLAY_BUST=$OVERLAY_BUST ===" && ls /app/src/lib/ | grep -i aether || echo "NO aether in src/lib" && echo "=== DIAG aether dir ===" && ls /app/src/lib/aether/ 2>&1 | head -15 || echo "NO_AETHER_DIR"
